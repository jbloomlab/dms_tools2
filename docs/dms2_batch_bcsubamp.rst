...
Overview
-------------
The ``dms2_batch_bcsubamp`` program can process the FASTQ reads generated by :ref:`bcsubamp` to count the frequencies of mutations at each site for a set of samples, and then summarize the results.

The ``dms2_batch_bcsubamp`` program simply runs :ref:`dms2_bcsubamp` for each of a set of samples listed in a batch file.
It then creates some summary plots in `Output files`_.
